<template>
  <div class="main">
    <h1 style="float: left;color:#e9eef3">食品管理</h1>
    ...
    >
      ...
      <el-table-column prop="dose" label="食品说明" width="100"/>
...
export default {
  ...
  methods: {
    getPersonList() {
      let self = this;
      httpRequest.prototype.getInitViewFoodPage(self.page).then(res => {
        if (isSuccess(res)) {
    ...
    handleClick(id) {
      ...
      httpRequest.prototype.getPersonnelUser(id).then(res => {
        if (isSuccess(res)) {
    ...
    setUserNotUse(id) {
      ...
      httpRequest.prototype.updateFood(data).then(res => {
        if (isSuccess(res)) {
          successMessage("操作成功");
          self.getPersonList();
        } else {
    ...
    onSubmit() {
      ...
      httpRequest.prototype.addFood(self.form).then(res => {
        if (isSuccess(res)) {
          successMessage("添加成功");
    ...
    Modify(id) {
      let self = this;
      httpRequest.prototype.findFoodById(id).then(res => {
        if (isSuccess(res)) {
          this.form = res.data;
    ...
    modifyUpdate() {
      ...
      self.form.password = self.password;
      httpRequest.prototype.updateFood(self.form).then(res => {
        if (isSuccess(res)) {
          successMessage("修改成功");
