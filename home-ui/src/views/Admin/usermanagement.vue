<template>
  <div class="main">
  <h1 style="float: left;color:#e9eef3">用户管理</h1>
  ...
  >
    ...
    <el-table-column prop="uuid" label="身份证号码" width="160"/>
...
export default {
  ...
  methods: {
    getPersonList() {
      let self = this;
      httpRequest.prototype.selectPage(self.page).then(res => {
        if (isSuccess(res)) {
    ...
    setUserNotUse(id) {
      httpRequest.prototype.setUserNotUse(id).then(res => {
        if (isSuccess(res)) {
          successMessage("操作成功");
        } else {
          errorMessage(res);
    ...
    onSubmit() {
      ...
      httpRequest.prototype.adminAddUser(self.form).then(res => {
        if (isSuccess(res)) {
          successMessage("添加成功");
    ...
    Modify(id) {
      let self = this;
      httpRequest.prototype.findUserById(id).then(res => {
        if (isSuccess(res)) {
          this.form = res.data;
    ...
    modifyUpdate() {
      ...
      self.form.password = self.password;
      httpRequest.prototype.update(self.form).then(res => {
        if (isSuccess(res)) {
          successMessage("修改成功");
